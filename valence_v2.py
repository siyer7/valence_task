...
nitask = nidaqmx.Task() 
nitask.do_channels.add_do_chan("Dev1/port0/line8:15",line_grouping=LineGrouping.CHAN_PER_LINE)
nitask.start()
...
expName = 'valence_final_v2'  # from the Builder filename that created this script
# information about this experiment
expInfo = {
    'subj': '',
    'practice?': '1 or nothing',
...
def setupData(expInfo, dataDir=None):
    ...
    if dataDir is None:
        dataDir = _thisDir
    filename = u'results/%s_subj-%s_difficulty-%s_order-%s_%s' % (expName, expInfo['subj'], expInfo['practice?'], expInfo['sess_type'], expInfo['date'])
    ...
    thisExp = data.ExperimentHandler(
        ...
        originPath='/Users/f0064z8/Documents/code_repos/valence_task/valence_version2_lastrun.py',
...
def setupDevices(expInfo, thisExp, win):
    ...
    if deviceManager.getDevice('valence_resp') is None:
        # initialise valence_resp
        valence_resp = deviceManager.addDevice(
            deviceClass='keyboard',
            deviceName='valence_resp',
        )
...
def run(expInfo, thisExp, win, globalClock=None, thisSession=None):
    ...
    # --- Initialize components for Routine "valence_instruct" ---
    valence_image = visual.ImageStim(
        win=win,
        name='valence_image', 
        image=None, mask=None, anchor='center',
        ori=0.0, pos=(0, 0), draggable=False, size=(1.2, 0.4),
        color=[1,1,1], colorSpace='rgb', opacity=None,
        flipHoriz=False, flipVert=False,
        texRes=128.0, interpolate=True, depth=-1.0)
    valence_text = visual.TextStim(win=win, name='valence_text',
        text='',
        font='Arial',
        ...
        languageStyle='LTR',
        depth=-2.0);
    valence_resp = keyboard.Keyboard(deviceName='valence_resp')
    ...
    # NEW
    # Load session configuration from CSV
    # assumes col 1 is the index
    sess_types_df = pd.read_csv('input_data/sess_types_df_v2.csv', index_col=0)
    block_order = sess_types_df.loc[expInfo['sess_type']]['block_order']
    block_order = eval(block_order)
    ...
    for thisBlock in blocks:
        ...
        # --- Prepare to start Routine "valence_instruct" ---
        # create an object to store info about Routine valence_instruct
        valence_instruct = data.Routine(
            name='valence_instruct',
            components=[valence_image, valence_text, valence_resp],
        )
        valence_instruct.status = NOT_STARTED
        continueRoutine = True
        # update component parameters for each repeat
        # Run 'Begin Routine' code from valence_code
        # Check if we should show instructions
        if block_id not in [0,3]:
            continueRoutine = False
        else:
            # Determine which image to show
            if block_order[block_id] < 3:
                img_path = 'input_data/rewardB.png'
            else:
                img_path = 'input_data/rewardA.png'
            
            # Update the image
            valence_image.setImage(img_path)
        valence_text.setText('Rules for the next 3 blocks are below.\nPress the up arrow to continue.')
        # create starting attributes for valence_resp
        valence_resp.keys = []
        valence_resp.rt = []
        _valence_resp_allKeys = []
        # store start times for valence_instruct
        valence_instruct.tStartRefresh = win.getFutureFlipTime(clock=globalClock)
        valence_instruct.tStart = globalClock.getTime(format='float')
        valence_instruct.status = STARTED
        thisExp.addData('valence_instruct.started', valence_instruct.tStart)
        valence_instruct.maxDuration = None
        # keep track of which components have finished
        valence_instructComponents = valence_instruct.components
        for thisComponent in valence_instruct.components:
        ...
        # --- Run Routine "valence_instruct" ---
        # if trial has changed, end Routine now
        if isinstance(blocks, data.TrialHandler2) and thisBlock.thisN != blocks.thisTrial.thisN:
            continueRoutine = False
        valence_instruct.forceEnded = routineForceEnded = not continueRoutine
        while continueRoutine:
            ...
            # *valence_image* updates
            
            # if valence_image is starting this frame...
            if valence_image.status == NOT_STARTED and frameN >= 0.0:
                # keep track of start time/frame for later
                valence_image.frameNStart = frameN  # exact frame index
                valence_image.tStart = t  # local t and not account for scr refresh
                valence_image.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(valence_image, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'valence_image.started')
                # update status
                valence_image.status = STARTED
                valence_image.setAutoDraw(True)
            
            # if valence_image is active this frame...
            if valence_image.status == STARTED:
                # update params
                pass
            
            # *valence_text* updates
            
            # if valence_text is starting this frame...
            if valence_text.status == NOT_STARTED and frameN >= 0.0:
                # keep track of start time/frame for later
                valence_text.frameNStart = frameN  # exact frame index
                valence_text.tStart = t  # local t and not account for scr refresh
                valence_text.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(valence_text, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'valence_text.started')
                # update status
                valence_text.status = STARTED
                valence_text.setAutoDraw(True)
            
            # if valence_text is active this frame...
            if valence_text.status == STARTED:
                # update params
                pass
            
            # *valence_resp* updates
            waitOnFlip = False
            
            # if valence_resp is starting this frame...
            if valence_resp.status == NOT_STARTED and frameN >= 0.0:
                # keep track of start time/frame for later
                valence_resp.frameNStart = frameN  # exact frame index
                valence_resp.tStart = t  # local t and not account for scr refresh
                valence_resp.tStartRefresh = tThisFlipGlobal  # on global time
                win.timeOnFlip(valence_resp, 'tStartRefresh')  # time at next scr refresh
                # add timestamp to datafile
                thisExp.timestampOnFlip(win, 'valence_resp.started')
                # update status
                valence_resp.status = STARTED
                # keyboard checking is just starting
                waitOnFlip = True
                win.callOnFlip(valence_resp.clock.reset)  # t=0 on next screen flip
                win.callOnFlip(valence_resp.clearEvents, eventType='keyboard')  # clear events on next screen flip
            if valence_resp.status == STARTED and not waitOnFlip:
                theseKeys = valence_resp.getKeys(keyList=['up'], ignoreKeys=["escape"], waitRelease=False)
                _valence_resp_allKeys.extend(theseKeys)
                if len(_valence_resp_allKeys):
                    valence_resp.keys = _valence_resp_allKeys[-1].name  # just the last key pressed
                    valence_resp.rt = _valence_resp_allKeys[-1].rt
                    valence_resp.duration = _valence_resp_allKeys[-1].duration
            ...
            if not continueRoutine:  # a component has requested a forced-end of Routine
                valence_instruct.forceEnded = routineForceEnded = True
                break
        ...
        # --- Ending Routine "valence_instruct" ---
        for thisComponent in valence_instruct.components:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # store stop times for valence_instruct
        valence_instruct.tStop = globalClock.getTime(format='float')
        valence_instruct.tStopRefresh = tThisFlipGlobal
        thisExp.addData('valence_instruct.stopped', valence_instruct.tStop)
        # Run 'End Routine' code from valence_code
        if continueRoutine:
            valence_image.tStopRefresh = tThisFlipGlobal
        # check responses
        if valence_resp.keys in ['', [], None]:  # No response was made
            valence_resp.keys = None
        blocks.addData('valence_resp.keys',valence_resp.keys)
        if valence_resp.keys != None:  # we had a response
            blocks.addData('valence_resp.rt', valence_resp.rt)
            blocks.addData('valence_resp.duration', valence_resp.duration)
        # the Routine "valence_instruct" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
        ...
        #print('trig10', end=', ')
        nitask.write(np.unpackbits(np.uint8(10), bitorder='little').astype('bool'))
        ...
        # index block then select 40 rows therafter
        trial_rows = [row + (block_order[block_id]) * 40 for row in range(40)]
        
        # new
        # index block then select 2/40 random rows thereafter
        if expInfo['practice?'] == '1':
            trial_rows = [row + (block_order[block_id]) * 40 for row in random.sample(range(40), 2)]
        ...
        trials = data.TrialHandler2(
            ...
            f'input_data/trials_patients_v2.csv', 
        ...
        for thisTrial in trials:
            ...
            #print('trig20', end=', ')
            nitask.write(np.unpackbits(np.uint8(20), bitorder='little').astype('bool'))
            ...
            #print('trig30', end=', ')
            nitask.write(np.unpackbits(np.uint8(30), bitorder='little').astype('bool'))
            ...
            #print('trig40', end=', ')
            nitask.write(np.unpackbits(np.uint8(40), bitorder='little').astype('bool'))
            ...
            #print('trig50', end=', ')
            nitask.write(np.unpackbits(np.uint8(50), bitorder='little').astype('bool'))
            ...
            while continueRoutine:
                ...
                # released
                    leftPressed = 0
                #    print('trig53', end=', ')
                    nitask.write(np.unpackbits(np.uint8(53), bitorder='little').astype('bool'))
                ...
                # pressed
                    ...
                    if marker_moved and not thrown:
                        thrown = True
                #        print('trig51', end=', ')
                        nitask.write(np.unpackbits(np.uint8(51), bitorder='little').astype('bool'))
                    if not leftPressed:
                #        print('trig52', end=', ')
                        nitask.write(np.unpackbits(np.uint8(52), bitorder='little').astype('bool'))
                ...
                # released
                    rightPressed = 0
                #    print('trig55', end=', ')
                    nitask.write(np.unpackbits(np.uint8(55), bitorder='little').astype('bool'))
                ...
                #        print('trig51', end=', ')
                    ...
                    if not rightPressed:
                #        print('trig54', end=', ')
                        nitask.write(np.unpackbits(np.uint8(54), bitorder='little').astype('bool'))
                ...
                #    print('trig56', end=', ')
                    nitask.write(np.unpackbits(np.uint8(56), bitorder='little').astype('bool'))
            ...
            #print('trig60', end=', ')
            nitask.write(np.unpackbits(np.uint8(60), bitorder='little').astype('bool'))
            ...
            #print('trig70', end=', ')
            nitask.write(np.unpackbits(np.uint8(70), bitorder='little').astype('bool'))
        ...
        txt1 = f'You gained {block_outcome} out of a possible 80 coins.\n\n'
        txt2 = f'Based on your precision and speed, you gained {block_bonus} out of a possible 40 bonus points.\n\n'
        txt3 = 'Press the up arrow to begin the next block.'
        block_end_txt = txt1 + txt2 + txt3
        
        #print('trig80', end=', ')
        nitask.write(np.unpackbits(np.uint8(80), bitorder='little').astype('bool'))
